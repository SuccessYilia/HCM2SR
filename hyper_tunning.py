...
class HyperTuning(object):
    def __init__(self, objective_function, logger_file, space=None, params_file=None, algo=tpe.suggest, max_evals=100):
        ...
        logging.basicConfig(level=logging.INFO, filename=logger_file)
    ...
    def params2list(params):
        ...
        for param_name in params:
            ...
            params_list.append(str(params[param_name]))
        return params_list
    ...
    def trial(self, params):
        ...
        params_info_output = 'begin to run parameters: %s' % str(params)
        print(params_info_output)
        self.logger.info(params_info_output)
        result_dict = self.objective_function(self.params2list(params))
        ...
        result_info_output = 'Current valid score: %s' % str(score) + '\n' \
                             + 'Best valid score: %s' % str(self.best_score) + '\n' \
                             + 'Best params: %s' % str(self.best_params) + '\n' \
                             + 'Best test result: %s' % str(self.best_test_result) + '\n'
        print(result_info_output)
        self.logger.info(result_info_output)
